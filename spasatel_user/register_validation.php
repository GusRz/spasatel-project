...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ruta_imagenes = "../uploads/uploads_user/";
    ...
    if (move_uploaded_file($imagen_frontal_tmp, $ruta_imagenes . $imagen_frontal_nombre) &&
        ...
        $check_query = "SELECT * FROM user WHERE type_id='$type_id' AND id='$id'";
        ...
        if ($check_result->num_rows > 0) {
            ...
            $query = "INSERT INTO user (id, type_id, nombres, apellidos, telefono_celular, email, password, imagen_frontal, imagen_trasera, fecha_registro) 
            ...
            if ($mysqli->query($query) === TRUE) {
                ...
                // Renombra los archivos con el ID user
                $newNameImagenFrontal = $id_user . "_" . $type_id . "_" . $id . "_" . uniqid() . "." . pathinfo($_FILES["imagen_frontal"]["name"], PATHINFO_EXTENSION);
                $newNameImagenTrasera = $id_user . "_" . $type_id . "_" . $id . "_" . uniqid() . "." . pathinfo($_FILES["imagen_trasera"]["name"], PATHINFO_EXTENSION);

                // Actualiza los nombres de las imágenes en la base de datos
                $update_query = "UPDATE user SET imagen_frontal='$newNameImagenFrontal', imagen_trasera='$newNameImagenTrasera' WHERE id_user='$id_user'";
